...
theorem simStepRelation_ok_sound {net : Net} (A B : State net)
  ...
  cases h with
  | ok _ hVF hR0 hR1 _ => exact validFloodFill_adb_ptc A B powered hVF hR0 hR1
...
theorem simStepRelation_ok_con {net : Net} (A B : State net)
  ...
  cases h with
  | ok _ hVF hR0 hR1 _ =>
    exact cap_adb_ptc_implies_con A B hA hVF.2.2.1 hR0 hR1
...
-- An adbError means no valid next state exists.
-- The unique ValidFloodFill state already fails ADB, so no candidate can satisfy ValidNextState.
theorem simStepRelation_adbError_no_valid {net : Net} (A : State net)
    (powered : net.nodes → Option Value) (requireStatic : Bool) (T : Transistor net.nodes)
    (h : SimStepRelation requireStatic A powered (.adbError T)) :
    ¬ ∃ B : State net, ValidNextState A B powered := by
  cases h with
  | adbError _ hVF hT_mem hgate hends =>
    intro ⟨B', hB'⟩
...
-- A ptcError means no valid next state exists.
-- The unique ValidFloodFill state already fails PTC, so no candidate can satisfy ValidNextState.
theorem simStepRelation_ptcError_no_valid {net : Net} (A : State net)
    (powered : net.nodes → Option Value) (requireStatic : Bool) (v : net.nodes)
    (h : SimStepRelation requireStatic A powered (.ptcError v)) :
    ¬ ∃ B : State net, ValidNextState A B powered := by
  cases h with
  | ptcError _ hVF _ hchange hdisconn =>
    intro ⟨B', hB'⟩
    have hVF' := validNextState_implies_validFloodFill A B' powered hB'
    have huniq := validFloodFill_uniqueness A _ B' powered hVF hVF'
    obtain ⟨-, -, -, -, -, hPTC⟩ := hB'
